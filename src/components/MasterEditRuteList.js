...
import DisplayMasterEditRuteList from "./DisplayMasterEditRuteList";

const MasterEditRuteList = () => {
  const [dRute, setDRute] = useState([]);
  const [infoRute, setInfoRute] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  const { id } = useParams(); // id rute
  console.log('mastereditrutelist', id);
...
export default MasterEditRuteList;
